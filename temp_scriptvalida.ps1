...
Import-Module -Name "C:\Users\i315767x900\Desktop\GroupMembers"
...
#Lista de nomes (Administrators, Administradores)
$gruposadm = "administrators"


<# CRIA AMBIENTE BASE - REMOVER COMENTARIO APOS SCRIPT01
echo $null > 'C:\Suporte Windows\Lista\baseline\aplicacao.txt'
echo $null > 'C:\Suporte Windows\Lista\baseline\dba.txt'
echo $null > 'C:\Suporte Windows\Lista\baseline\producao.txt'
echo $null > 'C:\Suporte Windows\Lista\baseline\default.txt'
mkdir "C:\Suporte Windows\Lista\baseline\personalizado\"
...
#> #CRIA NOME DAS VARIAVEIS DOS NOMES DE SERVIDOR - VOLTAR APOS CONCLUIR SCRIPT 01

<# CARREGA LISTA SERVERS PARA EXECUCAO SCRIPT01 - POR PASTAS - FULL - REMOVER APOS SCRIPT01
$serveribi = gci 'C:\suporte windows\Lista\baseline\BANCOIBI' -Recurse -Filter "*.txt"
$serverd98 = gci 'C:\suporte windows\Lista\baseline\D9803D01' -Recurse -Filter "*.txt"
$serverbca = gci 'C:\suporte windows\Lista\baseline\BRADESCOCARTOES' -Recurse -Filter "*.txt"
$serveramx = gci 'C:\suporte windows\Lista\baseline\AMEXDC' -Recurse -Filter "*.txt"

$serveribiSname = gci 'C:\suporte windows\Lista\baseline\BANCOIBI' -Recurse -Filter "*.txt"  |% {$_.name} |% {$_.split(".txt")} |findstr /R "[^0-9]"
$serverd98Sname = gci 'C:\suporte windows\Lista\baseline\D9803D01' -Recurse -Filter "*.txt" |% {$_.name} |% {$_.split(".txt")} |findstr /R "[^0-9]"
$serverbcaSname = gci 'C:\suporte windows\Lista\baseline\BRADESCOCARTOES' -Recurse -Filter "*.txt" |% {$_.name} |% {$_.split(".txt")} |findstr /R "[^0-9]"
$serveramxSname = gci 'C:\suporte windows\Lista\baseline\AMEXDC' -Recurse -Filter "*.txt" |% {$_.name} |% {$_.split(".txt")} |findstr /R "[^0-9]"



#> # FIM CARREGA LISTA SERVERS - POR PASTAS - FULL / SEM SEPARACAO (PARTE DE SCRIPT01)
...
#FUNCAO ALTERADA PARA SCRIPT01 INICIO
$funcao = Read-Host "O que deseja fazer:
(A) Listar Grupos (NAO ALTERADO EM SCRIPT01)
(B) Validar de acordo com a baseline - SCRIPT01 PERSONALIZADO
(C) Alterar/Corrigir Grupos - SCRIPT01 PERSONALIZADO
(D) Grupos especiais Backup/Coti (NAO ALTERADO EM SCRIPT01) 
"
switch ($funcao){
...
#FIM FUNCAO ALTERADA PARA SCRIPT01 INICIO
